...
QString LammpsGui::interactiveContentFor(int collection, int tutno)
{
    ...
    if (coll.key == QStringLiteral("softmatter")) {
        ...
        if (tutno == 5) return QStringLiteral(":/tutorials/silica-relax.json");
    }
    return {};
}
...
void LammpsGui::startInteractiveTutorial(const QString &path)
{
    ...
    connect(tutorialview, &TutorialView::markLines, textEdit, [this](const QStringList &texts) {
        if (texts.isEmpty())
            textEdit->clearMarkedLine();
        else
            textEdit->markLines(texts);
    });
